...
/** 动态列表（看板右栏和首页面板共用） */
export function ActivityList({ items, onSelect }: ListProps) {
  return (
      ...
      {items.map((a) => (
        <button key={a.id} className="rail-item" onClick={() => onSelect(a.task_id)}>
          ...
          <span className="rail-body">
            ...
            {a.kind === 'created'
              ? createAgentInfo(a) && (
                  <span className="rail-content rail-creator">{createAgentInfo(a)}</span>
                )
...
export function ActivityRail({ items, onSelect }: ListProps) {
  return (
